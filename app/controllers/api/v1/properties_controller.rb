      ...
      def show
        ...
        if @property
          ...
          if @property.lat.blank? == false && @property.long.blank? == false
            @near_properties = Property.where(status: ["Live Online Bidding", "Approve", "Best Offer"]).near([@property.lat, @property.long], 50000, units: :km).limit(4)
            if @near_properties.length < 4
              @near_properties = @near_properties + (Property.where(status: ["Live Online Bidding", "Approve", "Best Offer"]).where.not(id: @near_properties.ids).order(address: :asc).limit(4-@near_properties.length))
            end
          else
            @near_properties = Property.where(status: ["Live Online Bidding", "Approve", "Best Offer"]).order(address: :asc).limit(4)
          end
          @seller_pay_types = SellerPayType.all.order(:created_at)
          @show_instructions_types = ShowInstructionsType.all.order(:created_at)
