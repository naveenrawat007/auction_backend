  ...
  def perform(mailer_template_id, email)
    ...
    if mailer_template
      ...
      if mailer_template.code == "template1"
        ...
        BidMailer.out_bidded(offer_id, email, user_id, property_id).deliver
      elsif mailer_template.code == "template17"
        BidMailer.submitted(offer_id, email, user_id, property_id).deliver
      elsif mailer_template.code == "template18"
        BuyNowMailer.submitted(offer_id, email, user_id, property_id).deliver
      elsif mailer_template.code == "template19"
        BidMailer.highest_bidder_notification(offer_id, email, user_id, property_id).deliver
      elsif mailer_template.code == "template20"
        PropertyMailer.watchers_post_notification(property_id, user_id, email).deliver
